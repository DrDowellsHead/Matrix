...
START_TEST(test_dimensions_equal_invalid_cases) {
  s21_matrix A;
  s21_matrix B;
  ...
  ck_assert_int_eq(s21_dimensions_equal(NULL, &B), 0);
  ck_assert_int_eq(s21_dimensions_equal(&A, NULL), 0);
  ck_assert_int_eq(s21_dimensions_equal(NULL, NULL), 0);
  ck_assert_int_eq(s21_dimensions_equal(&A, &B), 1);
...
START_TEST(test_transpose_invalid_cases) {
  s21_matrix A;
  s21_matrix result;
...
START_TEST(test_calc_complements_null_cases) {
  s21_matrix A;
  s21_matrix result;
...
START_TEST(test_validate_matrix_invalid_cases) {
  s21_matrix A;

  ck_assert_int_eq(s21_validate_matrix(NULL), 1);

  A.matrix = NULL;
  A.rows = 2;
  A.columns = 2;
  ck_assert_int_eq(s21_validate_matrix(&A), 1);
  ...
  A.columns = 2;
  ck_assert_int_eq(s21_validate_matrix(&A), 0);
...
Suite *s21_transform_suite(void) {
  ...
  tcase_add_test(tc, test_transpose_invalid_cases);
...
Suite *s21_advanced_suite(void) {
  ...
  tcase_add_test(tc, test_calc_complements_null_cases);
  tcase_add_test(tc, test_inverse_matrix_null_A);
...
Suite *s21_helpers_suite(void) {
  ...
  TCase *tc = tcase_create("helpers");

  tcase_add_test(tc, test_validate_matrix_invalid_cases);
  tcase_add_test(tc, test_dimensions_equal_invalid_cases);
...
int main(void) {
  ...
  Suite *suites[] = {s21_create_remove_suite(),
                     s21_eq_suite(),
